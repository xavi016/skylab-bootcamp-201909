...
function pasapalabra() {
    ...
    alert("Bienvenid@ a Pasapalabra\n Para cada pregunta podras responder o en su defecto escribir pasapalabra." +
        "\nPara terminar el juego deberás contestar todas las preguntas o en su defecto escribir 'end'\n Buena Suerte!")

    do {
        resetGame(); //Sortea las preguntas y pone los contadores y validadores a cero
        do {

            //Este if comprueba, si se ha respondido la primera vuelta y si se ha acabado también con las que quedaban sin responder
            //En ese caso sale de las preguntas y hace el insert en el ranking.
            if (validacion.contadorVuelta == questions.length && noContestadas.length == 0) {
                break;
            }
            preguntasRosco();
    ...
    function preguntasRosco() {
        if (validacion.contadorVuelta < questions.length) { //La primera vuelta se preguntan por orden
            var respuesta = prompt(questions[validacion.contadorVuelta].question);
            validarPregunta(questions, respuesta, validacion.contadorVuelta);

        } else {//En la segunda vuelta se preguntan por orden las no respondidas.
            volverEmpezarRepetidas = false;
            do {
                for (var a = 0; a < noContestadas.length; a++) {
                    if (noContestadas[a].status == 0) {
                        var respuesta = prompt(noContestadas[a].question);
                        validarPregunta(noContestadas, respuesta, a);
                        if (noContestadas[a].status == 1) {
                            questions[questions.indexOf(noContestadas[a])].status == 1;
                            noContestadas.splice(a, 1);
                        } else if (noContestadas[a].status == -1) {
                            questions[questions.indexOf(noContestadas[a])].status == -1;
                            noContestadas.splice(a, 1);
                        }
                    }
                }
            } while (noContestadas.length > 0);
    ...
    function mostrarRanking() {
        var text = "Nombre    Acertadas    Falladas\n";
        ...
        ranking.forEach(function (element) {
            text += element.nombre + "     ---->     " + element.aciertos + "     ---->     " + element.fallos + "\n";
        });
    ...
    function resetGame() {
        jugador.nombre = prompt("Bienvenid@ a Pasapalabra\n Cómo te llamas?");
