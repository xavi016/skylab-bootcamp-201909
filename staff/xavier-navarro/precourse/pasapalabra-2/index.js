let a = [
  ...
  // Current position of the letter
  let currentLetter = 0
  ...
  function resetGame(){
    ...
    // reset rosco
    roscoLetter = document.querySelectorAll(".letter")
    roscoLetter.forEach(element => {
    ...
    while (rankingList.hasChildNodes()) {
      rankingList.removeChild(rankingList.firstChild);
    }
    // reset countdown
    document.getElementById("countdown").textContent = 150
    // delete answer
  ...
  function stopTimer() {
    clearInterval(roscoTimer)
  }

  function countScore(recordQuestSelected){
    let score = 0
    for(let i = 0; i < questions.length; i++ ){
      if(recordQuestSelected[i] !== undefined){
  ...
  function printRanking(ranking){
    ...
    let li
      ...
      for(let i = 0; i < ranking.length; i++){
        ...
        li.classList.add("ranking--position")
        li.appendChild(document.createTextNode("Player name: "+ ranking[i]["player"]+" Score: "+ranking[i]['score']+" Time left: "+ranking[i]['timeLeft']+" seconds"))
        ol.appendChild(li)
      ...
      if(rank.length == 0){
        li = document.createElement("li")
        li.classList.add("ranking--position")
        li.appendChild(document.createTextNode("There are no records."))
  ...
  function endGame(){
    stopTimer()
    document.getElementById("container-quest").style = "display:none"
  ...
  function gameComplete() {
    ...
    playerScore['timeLeft'] = parseInt(document.getElementById("countdown").textContent)
  ...
  function checkAnswer(){
    ...
    let questNum = recordQuestSelected[currentLetter].number
    if(answer !== null && answer !== undefined) answer = answer.toLowerCase()

    switch (answer) {
      case questions[currentLetter]['questions'][questNum]['answer'].toLowerCase():
          questions[currentLetter]['questions'][questNum]['status'] = 1
          // console.log(questions[currentLetter]["letter"].toLocaleUpperCase()+' - Correct, you have 1 Point!')
          document.querySelectorAll(".letter")[currentLetter].classList.add("correct")
          score += 1
      ...
      default:
        questions[currentLetter]['questions'][questNum]['status'] = -1
        document.querySelectorAll(".letter")[currentLetter].classList.add("incorrect")
        // console.log('Incorrect! '+questions[currentLetter]['questions'][questNum]['question']+'. Answer: '+questions[currentLetter]['questions'][questNum]['answer'])
        document.getElementById("answerQuestion").innerHTML = '<span class="label--incorrect">Incorrect!</span><br><br> '+questions[currentLetter]['questions'][questNum]['question']+'. <strong>Answer: '+questions[currentLetter]['questions'][questNum]['answer']+"</strong><h3>Next question:</h3>"
        break;
    }
  ...
  function askQuestions(){
    ...
    if(firstLapDone){
      isComplete = roscoComplete()
    }
    
    if(!isComplete){
      // comprueba si se ha completado el rosco
      if(recordQuestSelected.length !== totalLetters) {
        // seleciona pregunta
        questNum = randomNumber(numQuestion)
        // Almacena la letra y la pregunta
        let questSelected = {letter: questions[currentLetter]['letter'], number: questNum}
        recordQuestSelected.push(questSelected)
        // indica que pregunta ha sido seleccionada
        questions[currentLetter]['questions'][questNum]['selected'] = 1
      }else{
        //get number of question
        questNum = recordQuestSelected[currentLetter]['number']
      }
      if(!firstLapDone){
        if(questions[currentLetter]['questions'][questNum]['status'] === 0){
          question = questions[currentLetter]['questions'][questNum]['question']+"."
  ...
  function playerName(event){
    ...
    user = event.target.user_name.value;
    document.getElementById("user").textContent = user
    document.getElementById("icon").style = "display:block";
    document.getElementById("player-form").style = "display:none";
    playerScore = {player: user, score: 0, ranking: 0, timeLeft: 0}
    // It's the position in the array of the current letter
    document.getElementById("container-quest").style = "display:block";
    roscoTimer = setInterval(timer, 1000)
    askQuestions()
  }
