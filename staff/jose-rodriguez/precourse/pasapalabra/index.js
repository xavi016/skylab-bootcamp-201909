...
function getNewRosco() {
    ...
    for (var i=0; i<27; i++) {
        ...
        rosco[i] = questions[i][randNum]
    }
    return rosco;
}

function roscoCompleto(rosco) {
...
function pasapalabra (){
    ...
    while (continuePlaying) {
        player = new Player;
        var rosco = getNewRosco ();
        console.log (`Bienvenido a Pasapalabra ${player.name}!!!`)
        console.log("El juego consiste en responder correctamente a cada definición del rosco")
        console.log("Si te atascas en una palabra... no te preocupes, introduce 'pasalabra' y ya la acertaras luego!")
        console.log("Empecemos!!")
        var i=0;
        while (!roscoCompleto(rosco) && !endGame) {
            ...
            if (i>=27) {
                ...
                if (rosco[i].status === 0) {
                console.log(rosco[i].question);
                respuesta = prompt(rosco[i].question,"pasapalabra").toLowerCase();
                console.log(`Esta es tu respuesta: ${respuesta}`)
                    switch (respuesta) {
                        case rosco[i].answer:
                            rosco[i].status = 1;
                            player.score +=1;
                            ...
                            break;
                        case "pasapalabra":
                            console.log("Siguente letra");
                            i++;
                            break;
                        case ("end"):
                            console.log("Ha finalizado la partida. Hasta la próxima")
                            endGame = true;
                            continuePlaying = false;
                            break;
                        default:
                            rosco[i].status = -1;
                            console.log ("NOOO!! Respuesta incorrecta!");
                            console.log(`La respuesta correcta es: ${rosco[i].answer}!!!`)
                            i++;
                            break;
                    }
        ...
        if (roscoCompleto(rosco)) {
            console.log("Se acabó!!");
            ranking.push(player)
            ranking.sort(function(a,b){return b.score-a.score})
            console.log(`Has acertado un total de ${player.score} palabras!!`)
            console.log("Este es el ranking actual: ")
                for (var i=0; i<ranking.length; i++) {
                    console.log(`En ${i+1}º puesto: ${ranking[i].name} -> ${ranking[i].score} aciertos!`)  
                }
            console.log("Gracias por jugar a Pasapalabra!!")
        }
