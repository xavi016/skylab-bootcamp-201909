var tablaDefiniciones = [
    ...
    [   { letra: "g", palabra: "gotico", status: 0, definicion: "CON LA G. Estilo arquitectónico que resulta de la evolución del románico, caracterizado por el arco ojival y la bóveda de aristas."},
        { letra: "g", palabra: "guión", status: 0, definicion: "CON LA G. Texto a seguir por los actores de una obra teatral o película."},
...
//muestra por pantalla el número de aciertos y fallos del jugador.
//además, retorna el número de aciertos para que pueda ser registrado.
function mostrarAciertos(rosco) {
    ...
    console.log("Has acertado "+aciertos+" palabras y has cometido "+fallos+" fallos.")
    return aciertos;
}
function pasapalabraPro() {
    ...
    while (continuePlaying) {
        ...
        var rosco=crearRosco();
        
        console.log('\tHola '+player.name+'!! Bienvenido a PasapalabraPRO!!');
        console.log('\tVamos a repasar las reglas del juego:');
        console.log('\tEl objetivo del juego es acertar todas las palabras que comiencen o contengan cada letra del\
\n\tabecedario según se indique en cada caso en la definición: CON LA x o CONTIENE LA x.\
\n\t\t - Las palabras serán siempre sin acentos ni signos de puntuación.\
\n\t\t - No te preocupes por las MAYUS/MINUS, te entenderemos igual ;)\
\n\t\t - Puedes "pasar" de una letra si no quieres arriesgarte a fallar y dejarla para luego. Sólo debes escribir "PASAPALABRA"\
\n\t\t - Puedes finalizar el juego en cualquier momento indicando la palabra "END"');
        console.log('\n\nMuy bien, una vez aclaradas las reglas... Comienza el Juego!: ');

        while(!endGame) {
            while( !roscoCompleto(rosco) && !endGame) {
                if (rosco[index].status === 0) {
                    console.log(rosco[index].definicion);
                    respuesta=null;                
                    ...
                    switch (respuesta) {
                        ...
                        case rosco[index].palabra:
                                ...
                                break;
                        default:
                                console.log ("INCORRECTO!! La respuesta correcta era "+rosco[index].palabra);
                                rosco[index].status = -1;
                    }
                }
                index++;
